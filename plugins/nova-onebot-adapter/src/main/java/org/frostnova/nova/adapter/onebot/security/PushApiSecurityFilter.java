 ...
 * 受不受保护按框架匹配口径认定：请求路径按框架的 PathPattern 口径解析与比对——解析不了的
 * 路径默认拒绝；比对不上任何已登记推送接口的路径直接放行，不影响插件自行注册的其他接口；
 * 比对得上的请求一律过闸。至于某种具体写法（尾斜杠折回与否等）框架会不会路由过来，
 * 由框架自身的匹配配置决定，这里不预先担保。
 */
@Slf4j
