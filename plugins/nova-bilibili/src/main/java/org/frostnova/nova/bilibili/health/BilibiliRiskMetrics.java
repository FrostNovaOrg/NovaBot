...
public class BilibiliRiskMetrics {
     ...
     * <p>
     * 顶到这个数之后最老的记录被挤出去，{@link #count} 随之封顶，读数到顶时真值可能更大。
     * 挤掉的条数另记在 {@link #overflow}：启动以来累计、不分窗口，只说明挤过、挤了多少，
     * 不能与任何窗口读数相加。
     */
    ...
    public enum Kind {
         ...
         * <p>
         * 三类来源共用这一计数：protobuf 报文里字段表以外的字段号、HTTP 应答里已知表以外的顶层键、
         * 以及直播间消息里不认识的枚举取值（互动类型／货币／大航海档）。
         * <p>
         * 出现新字段不等于坏了，但要看得见：取值仍照常、事件仍照发，只是那个新字段被跳过，
         * 或那条消息按原来的规则丢掉。按名去重，detail 写计数、种数与首见时刻；
         * 枚举取值那一档带截到 32 字符的值，另两档不写取值。
         */
        UNKNOWN_FIELD("未知字段"),
