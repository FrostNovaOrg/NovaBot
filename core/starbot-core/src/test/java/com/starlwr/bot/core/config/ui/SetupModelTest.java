 ...
 * 要点出「插件步没做完不算齐」得先装上一个带向导步的插件再半途停下。而放行条件写反了
...
class SetupModelTest {
     ...
     */
    private static final String FIXTURE = "core/starbot-core/src/test/resources/frontend/setup-model-fixture.mjs";

    @Test
    @DisplayName("放行条件、进度条、落点、完成度与初始值逐格与预期相同")
    void stepMachineBehavesAsSpecified() throws IOException, InterruptedException {
        FrontendFixture.run(FIXTURE, "初始设置各步");
    }
}
