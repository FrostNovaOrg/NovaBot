...
public class NapCatCredentialService {
     ...
     * <p>
     * 取值＝<b>正常用法的上界 + 1</b>。正常用法有三个触发源：引导页取一把、
     * 那把不管用时重换一把、内层凭据过期时续登层换一把——上界是 <b>3</b>，所以这里取 4。
     * <p>
     * 🔴 <b>这里原先写的是 3，而那时它已经不对了。</b>
     * 续登层上线之前上界确实是 2，3 是「上界 +1」；续登层成了第三个触发源之后，
     * 上界变成 3 而这个数没跟着动——于是余量归零：<b>正常用法自己就会撞闸</b>，
     * 这道闸从「拦失控循环的」退化成「正常用法的天花板」。
     * 而撞闸与「NapCat 真的换不出来」在使用者眼里长得一模一样。
     * 改这个数时<b>先数清楚今天有几个触发源</b>，别照抄旧的 +1。
     * <p>
