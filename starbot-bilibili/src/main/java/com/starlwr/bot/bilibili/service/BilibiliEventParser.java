...
public class BilibiliEventParser {
     ...
     * 分支照写，见下面各自的说明。
     * <p>
     * ⚠️ <b>关注/分享样本难攒的首要原因是连接方式，不是运气</b>——
     * 2026-08-11 用同一直播间同一时段的登录态/匿名对照查实（{@code t3arch} 那对日志）：
     * <table border="1">
     *   <caption>同房同时段的两条连接</caption>
     *   <tr><th>连接</th><th>进房</th><th>关注</th><th>分享</th><th>带 uid 比例</th></tr>
     *   <tr><td>匿名</td><td>232</td><td>0</td><td>0</td><td><b>4%</b></td></tr>
     *   <tr><td>登录态</td><td>2117</td><td>4</td><td>1</td><td><b>100%</b></td></tr>
     * </table>
     * <b>匿名连接收到的进房约 96% 是打码的</b>（昵称形如 {@code 杨***}、无 uid）。
     * 上面那 2117 条进房样本正是取自登录态那一侧。
     * <p>
     * ⚠️ <b>但别把「慢」读成「不可能」</b>：匿名<b>能</b>攒到带 uid 的关注/分享，
     * 只是同样时长下产出约为登录态的 <b>1/25</b>。
     * 语料现有 5 条分享里 <b>4 条正是匿名攒出来的</b>。
     * 所以攒样本慢时，<b>该考虑的是要不要换连接方式，而不是断定这条路走不通</b>。
     * <p>
     * 平台还会把这两种互动折叠成 {@code DM_INTERACTION}
     * type=103/105（"N 人关注了主播" / "N 人分享了直播间"），
     * <b>聚合形态只有人数、没有 uid，也就不会走到这个方法</b>。
     * <b>而这很可能正是同一件事的另一面</b>：2026-08-11/12 做过同一直播间、同一次分享的对照——
    ...
    private StarBotBaseLiveEvent parseInteractV2(JSONObject data, LiveStreamerInfo source) {
        ...
        return switch (msgType.intValue()) {
            ...
            // 分享：5 条样本、跨 4 个直播间、5/5 带 uid，字段布局与关注逐条相同。
            // ⚠️ 但 n=5 仍按**实验性**对外标注：若存在「只在分享里出现、频次 30%」的字段，
            // n=5 漏检概率约 17%，而关注 n=10 是 2.8%——这就是两者一个转正一个留标的分界。
            // 攒够 8 条后转正并删掉这段（4.3.1 机制现成）
