...
- **归档必须发生在下播那一刻**。本场数据会在**下次开播时清零**，
  ...
  程序可能在两场之间重启
- **还有第二个归档时机：开播。** 程序在一场直播进行中崩溃时那一场没有下播事件，
  于是从未被归档，而下一次开播的清零会把它连同全部统计一起抹掉。
  所以 `LiveSessionRecovery` 在**清零之前**先检查「状态还挂在在播、开播时间不是这一次的」，
  命中就按「异常中断未闭合」归档。
  <br>**为什么不在启动那一刻判**：启动时并不知道那一场结束了没有——主播可能还在播
  （此时不该归档），也可能早就下播了（此时必须归档）。问平台要当前状态得各平台各写一套，
  而备用轮询是可以关掉的。开播这个时刻则必然到来、且信息完备
- **停机缺口按全局区间存**，不按主播存：停机是进程层面的事，
  同时监听 10 位主播时 10 场直播共享同一段缺口，各自的报告只截取与自己场次重叠的部分。
...
|---|---|---|---|
| 控制台页 | `config/ui/page/ConsolePageProvider` | `BilibiliConsolePageProvider` 与页面脚本 `config-ui-pages/bilibili.js`（starbot-bilibili）；主播页 `StreamersConsolePageProvider`（starbot-novabot-console） | 往控制台添自己的页；挂在连接页、设置页、顶级页、首页卡还是向导步骤由 `ConsolePageSlot` 申报；除 `script()` 外可再报 `assets()`（同目录其它 `.js`，按登记名取）。顶级页的 `refresh` 会收到 `{sub, tail}`（地址栏第二、三段） |
