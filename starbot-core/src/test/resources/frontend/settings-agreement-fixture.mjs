...
const ui = join(repo, 'starbot-core/src/main/resources/config-ui');
...
await section('正文只此一份', async () => {
  ...
  eq(existsSync(join(repo, 'build.sh')) && existsSync(join(repo, 'pom.xml')), true,
    '仓根定位对了，否则下面那一扫扫的是别处');
  sweep(repo);
  eq(carriers, ['starbot-core/src/main/resources/config-ui/agreement.txt'],
...
await section('协议三键归卡片', async () => {
  ...
  const keys = readFileSync(
    join(repo, 'starbot-core/src/test/resources/configuration-baseline/config-keys.txt'), 'utf8')
    .split('\n')
