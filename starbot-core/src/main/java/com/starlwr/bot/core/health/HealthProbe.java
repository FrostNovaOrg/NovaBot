...
public interface HealthProbe {
     ...
     * 本探针量的是不是「账号登录态」
     * <p>
     * 登录掉了与别的异常不是一回事：别的异常多半会自己恢复，而登录失效要人去扫码，
     * 不去就一直不好。时间线因此把它单列一类，好让使用者一眼看见「这条要我动手」。
     * <p>
     * 由探针自己声明而不是让调用方按名字认：按名字认的判据在探针改个显示名的那天
     * 静默失效，失效方向还是「从此再也认不出登录失效」。
     * @return 量的是登录态时返回 true
     */
    default boolean loginState() {
        return false;
    }
